...
hist(sqrtusAir$manu, main="sqrt manu", xlab="") #log better
hist(sqrtusAir$popul, main="sqrt popul", xlab="") #log better
hist(sqrtusAir$wind, main="sqrt wind", xlab="") # does not need transformation  
...
hist(logusAir$SO2, main="log SO2", xlab="") 
hist(logusAir$temp, main="log temp", xlab="") 
hist(logusAir$manu, main="log manu", xlab="") 
hist(logusAir$popul, main="log popul", xlab="") 
hist(usAir$wind, main="raw wind", xlab="")
...
```{r}
transusAir = usAir %>%
  mutate(logSO2 = log(SO2), logtemp = log(temp), logmanu = log(manu), logpop = log(popul)) %>%
...
I log transformed `SO2`, `temp`, `manu`, and `popul`.  The rest were left as is, log and square root transformations exaggerated skewness. 
...
```
**_**
Based on the scree plot, it seems like we can keep the first three since they explain the majoritiy of the variation. 
...
Based on the latent root criterion, we should keep the first three components since after that eigenvalues are < 1 and prove to explain more of the variance. 
\newline
...
The axes indicate how strongly each characteristic (variable vector) influence the variation (principal components). The x axis may be named as "Covariance between Component 1 and the original data" and the same with the y-axis. 
...
My dataset meets the assumption of multivariate normality, independence, are random and 
...
### Selecting the variables I will be working with 
```{r}
phe.shrt = phe %>%
...
Next, apply a z-score standardization:
```{r results='hide'}
Zphe<-scale(phe.sqrt[, 4:7]) #especially important to scale bc in very different units now 
```

### Running the PCA:
You are going to use the package `princomp` function in the stats package.
```{r}
phe_pca <- princomp(Zphe, cor = F)
```
...
#### Determining number of axes to keep
**Scree plot**
You now have 4 PC axes. Which ones give us vital information and which ones can we toss?  
```{r}
plot(phe_pca, type="lines")
```

**Latent root criterion (i.e., keep components with eigenvalues > 1)** and the **relative percent variance criteria**.  
Check the summary of the PCA explore this: 
```{r}
summary(phe_pca)
```
It seems that the first axis explains most of the variation

**Significance of factor loadings**
Another method for determining significance of factor loadings is bootstrapping. (see line 35 for more info)
```{r}
sigpca2<-function (x, permutations=1000, ...)
{
  pcnull <- princomp(x, ...)
...
sigpca2(Zphe, permutations=1000) # Use function with the scaled data as input
```
...
Plot out the factor loadings for the first 2 PC axes:
```{r fig.height= 7, fig.width= 7}
plot(phe_pca$loadings, type="n", xlab="PC 1, 39%", ylab="PC 2, 21%", ylim=c(-.8,.8), xlim=c(-.6,.6))
text(phe_pca$loadings, labels=as.character(colnames(Zphe)), pos=1, cex=1)
```
...
Let’s now plot the PC score for each sample (city):
```{r fig.height= 7, fig.width= 7 }
plot(phe_pca$scores, type="n",xlab="PC 1, 39%", ylab="PC 2, 21%", ylim=c(-2.5,4), xlim=c(-4,8))
text(phe_pca$scores, labels=as.character(rownames(Zphe)), pos=1, cex=1)
```

And now all together in a `biplot`:
```{r fig.height= 7, fig.width= 7}
biplot(phe_pca$scores, phe_pca$loading, xlab="PC 1, 39%", ylab="PC 2, 21%", expand= 4, ylim=c(-6.5,6), xlim=c(-4,7.5))
```

To replace observation ID with a symbol:
```{r fig.height= 7, fig.width= 7}
biplot(phe_pca$scores, phe_pca$loading, expand= 4, xlabs= rep("*",41), xlab="PC 1, 39%", ylab="PC 2, 21%", ylim=c(-6.5,6), xlim=c(-4,7))
```
...
```{r}
eigVec<-as.matrix(eig$vectors[,1:2])
rownames(eigVec) <- rownames(cov(Zphe))
```

Then simply multiply each eigenvector times the matrix of standardized observation values (Zphe) and plot!
```{r fig.height= 7, fig.width= 7}
scores<-t(rbind(eigVec[,1]%*%t(Zphe),eigVec[,2]%*%t(Zphe)))#hand calculated scores
# and plot
biplot(scores, eigVec, xlab="PC 1, 39%", ylab="PC 2, 21%", expand= 4, ylim=c(-6.5,6), xlim=c(-4,7.5))
```
